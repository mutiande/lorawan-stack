	...
	return func(lastUpMsg *ttnpb.UplinkMessage, downMsg *ttnpb.DownlinkMessage) (err error) {
		...
		case ttnpb.MType_JOIN_ACCEPT:
			...
			if dev.LoRaWANVersion.Compare(ttnpb.MAC_V1_1) >= 0 {
				copy(key[:], dev.GetRootKeys().GetNwkKey().GetKey())
			} else {
				copy(key[:], dev.GetRootKeys().GetAppKey().GetKey())
			}
			...
			if dev.LoRaWANVersion.Compare(ttnpb.MAC_V1_1) >= 0 && joinAcceptPayload.OptNeg {
				...
				copy(appKey[:], dev.GetRootKeys().GetAppKey().GetKey())
				...
				copy(nwkKey[:], dev.GetRootKeys().GetNwkKey().GetKey())
		...
		case ttnpb.MType_UNCONFIRMED_DOWN, ttnpb.MType_CONFIRMED_DOWN:
			...
			if dev.LoRaWANVersion.Compare(ttnpb.MAC_V1_1) < 0 {
				var key types.AES128Key
				copy(key[:], dev.Session.SessionKeys.GetFNwkSIntKey().GetKey())
				...
				var key types.AES128Key
				copy(key[:], dev.Session.SessionKeys.GetSNwkSIntKey().GetKey())
			...
			if macPayload.FPort == 0 {
				copy(key[:], dev.Session.SessionKeys.GetNwkSEncKey().GetKey())
			} else {
				copy(key[:], dev.Session.SessionKeys.GetAppSKey().GetKey())
			}
